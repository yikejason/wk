...
class ConditionSearch extends React.Component {
    ...
    GetRegionList = (rid) => {
        return fetch(`https:${basicConfig.development_api}/base/v3/Web/GetRegionList?rid=${rid || 0}&token=${sessionStorage.getItem('token')}`)
            .then(res => res.json())
            .then(res => {
    ...
    getSearchValue = () => {
        this.props.form.validateFields((err, fieldsValue) => {
            ...
            if (gid.length == 0 && type == '2') return message.info('暂未查询到相关结果')
    ...
    handleAutoSearch = (name) => {
        ...
        this.timer = setTimeout(() => {
            fetch(`https:${basicConfig.development_api}/mcs/v3/ConfigWeb/GetGroupList?keyWord=${name}&rid=${rid}&token=${sessionStorage.getItem('token')}`)
                .then(res => res.json())
                .then(res => {
                    if (!res) return message.error('res为空')
                    if (res.Ret === 0) {
                    ...
                    >
                        {getFieldDecorator('type', {initialValue: type})(
                            <Select
                                ...
                                placeholder="选择类型"
                            >
                                <Option value="1">通用版</Option>
                                <Option value="2">定制版</Option>
                            </Select>
                        )}
                    </FormItem>
                    {type == '2' && <FormItem
