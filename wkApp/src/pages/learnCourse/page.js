...
export default class LearnCourse extends React.Component {
    ...
    //推荐
    Recommend = (obj) => {
      api.Recommend({ body: { coursePeriodID: this.coursePeriodID, ContentType: '1', Content: JSON.stringify(obj) } }).then(res => {
        if (res.Ret !== 0) Toast.info(res.Msg, 1)
      })
    };
    ...
    play() {
        ...
        let businessParams = [];
        for (let i = 0; i < 5; i++) {
            businessParams.push({ "Time": Math.round(Math.random() * 4000), "Pause": true, "Msg": "" });
        }
        ...
        const extraparams = encodeURIComponent(`?CoursePeriodID=${this.coursePeriodID}&PlayRecodeID=${PlayRecodeID}`)
    ...
    // 判断是老师端还是学生端
    showButton = () =>{
        console.log(isTeacherPortFunc())
        const {
            IsCanEvaluate,
            errorMessage,
        } = this.state;
        // 学生端
        if (isTeacherPortFunc()){
             return (
                 <div>
                     {IsCanEvaluate && <ModalComp onSure={this.handleSure}> <span className={style.comment}>评一评</span></ModalComp>}
                     {!IsCanEvaluate && <div onClick={() => Toast.info(errorMessage, 1)}><span className={style.comment1}>评一评</span></div>}
                 </div>
             )
        }else {
            return (
                <div>
                    {IsCanEvaluate && <ModalComp onSure={this.handleSure}> <span className={style.comment3}>评一评</span></ModalComp>}
                    {!IsCanEvaluate && <div onClick={() => Toast.info(errorMessage, 1)}><span className={style.comments}>评一评</span></div>}
                     <span onClick={this.handleClickRecommend} className={style.recommended}>推荐学习</span>
                </div>
            )
        }
    };
    render() {
        ...
        return <div>
            ...
            <div className={style.top}>
                <img src={CoursePackagePic} alt=""  className={canPlay ? null : style.gray} />
            ...
            <div className={style.footer}>
                <div>
                    <div className={style.stydyicon}>
                        ...
                        <a href="javascript:void(0);" className={style.shareBox} onClick={this.share}>
