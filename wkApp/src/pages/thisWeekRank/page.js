...
import * as globleFn from '../../utils/globalFn'
...
const jfPic = (RuleRemind, is50) => {
   return (
        <div className={styles.card} style={{textAlign: 'center'}} >
        <img style={{width: '50%'}} src={require(`../../public/assets/jf${is50 ? 50 : 10 }.png`)} alt=""/>
...
export default class Home extends React.PureComponent {
    ...
    setLoading = (RuleRemind, is50) => {
        if (this.state.loading) {
             return null;
        } else {
            return (<div className={styles.body}>
                {nodata()}
                {jfPic(RuleRemind, is50)}
                </div>);
        }
    }
    ...
    isPupil() {
        ...
        && globleFn.isPupil(JSON.parse(sessionStorage.getItem('user')).PhaseID);
    }
  

    render() {
        const {data: {PointRank, LearnSecondsRank, LearnDaysRank, HabitRank, RuleRemind, WeekIsLearn, TodayIsGetPoint}} = this.state;
       
         return (<div>
            {
                !WeekIsLearn ? this.setLoading(RuleRemind, TodayIsGetPoint) :
                    <div>
                                ...
                                <ul>
                                     {<li>
                                        ...
                                        <div className={styles.card_content}>
                                            <div>
                                                <span>积分</span>
                                                <span>{PointRank && PointRank.Num && `${PointRank.Num} 分`}</span>
                                                <span>{PointRank && PointRank.Rank ? `第${PointRank.Rank}名` : '暂无排名'}</span>
                                            </div>
                                    ...
                                    {this.isPupil() &&  <li>
                                        ...
                                        <div className={styles.card_content}>
                                            <div>
                                                <span>好习惯</span>
                                                <span>{HabitRank && HabitRank.Num && `${HabitRank.Num} 天`}</span>
                                                <span>{HabitRank && HabitRank.Rank ? `第${HabitRank.Rank}名` : '暂无排名'}</span>
                                            </div>
                                    ...
                                    {<li>
                                        ...
                                        <div className={styles.card_content}>
                                            <div>
                                                <span>连续学习</span>
                                                <span>{LearnDaysRank && LearnDaysRank.Num && `${LearnDaysRank.Num} 天`}</span>
                                                <span>{LearnDaysRank && LearnDaysRank.Rank ? `第${LearnDaysRank.Rank}名` : '暂无排名'}</span>
                                            </div>
                                    ...
                                    {<li>
                                        ...
                                        <div className={styles.card_content}>
                                            <div>
                                                ...
                                                <span>{LearnSecondsRank && LearnSecondsRank.Rank ? `第${LearnSecondsRank.Rank}名` : '暂无排名'}</span>
                                            </div>
                        ...
                        {jfPic(RuleRemind, TodayIsGetPoint)}
                    </div>
            }
