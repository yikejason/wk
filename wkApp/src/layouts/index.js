...
export default class extends React.PureComponent {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.history.location.pathname !== this.pathname) {
            ...
            setTimeout(() => setTitleFunc(nextProps.history.location.pathname))
        }
    ...
    componentDidMount() {
        if (window.location.href.indexOf('goBack') !== -1) {
          ...
          sessionStorage.setItem('UCUX_OCS_AccessToken', token);
            sessionStorage.setItem('goBack', '1');
        }
        //去掉分享按钮
        setTimeout(() => showAppMenuFunc(), 300)
        ...
        if (navigator.userAgent.indexOf('UCUX_WebBrowser') === -1) {
            ...
            if(sessionStorage.getItem('UCUX_OCS_AccessToken')){this.judgeLinkTo()}
        }
        setTimeout(() => setTitleFunc(this.props.history.location.pathname))
    }
